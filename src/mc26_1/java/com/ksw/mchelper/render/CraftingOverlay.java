...
public class CraftingOverlay {
    ...
    // ==================== 渲染 ====================
    private static void renderRecipeBox(GuiGraphicsExtractor g, Minecraft mc,
        ...
        g.text(mc.font, "§b🔨 合成配方" + suffix, textX, textY, TEXT_TITLE, true);
        textY += LINE_HEIGHT;
    ...
    private static void renderCraftingGrid(GuiGraphicsExtractor g, Minecraft mc,
        ...
        for (int row = 0; row < entry.gridSize; row++) {
            for (int col = 0; col < entry.gridSize; col++) {
                ...
                if (idx < entry.ingredients.size() && !entry.ingredients.get(idx).isEmpty()) {
                    g.item(entry.ingredients.get(idx), px + 1, py + 1);
                }
            }
        }
        ...
        g.text(mc.font, "§7➜", arrowX, arrowY, TEXT_GRAY, true);
        ...
        g.item(entry.result, resultX + 1, resultY + 1);
        if (entry.result.getCount() > 1) {
            g.itemDecorations(mc.font, entry.result, resultX + 1, resultY + 1);
        }
    }

    private static void renderSmeltingRecipe(GuiGraphicsExtractor g, Minecraft mc,
                                              RecipeEntry entry, int textX, int textY) {
        ...
        g.text(mc.font, typeLabel, textX, textY, TEXT_GRAY, true);
        textY += LINE_HEIGHT + 2;
        ...
        if (!entry.ingredients.isEmpty()) {
            ...
            g.item(input, textX + 1, textY + 1);
            g.text(mc.font, "§7 ➜ ", textX + SLOT_SIZE + 2, textY + 4, TEXT_GRAY, true);
            int rx = textX + SLOT_SIZE + 22;
            g.fill(rx, textY, rx + SLOT_SIZE - 2, textY + SLOT_SIZE - 2, 0x60000000);
            g.item(entry.result, rx + 1, textY + 1);
            if (entry.experience > 0) {
                g.text(mc.font, String.format("§6+%.1f 经验", entry.experience),
    ...
    private static void drawBorder(GuiGraphicsExtractor g, int x, int y, int width, int height) {
