...
public class MinimapOverlay {
    ...
    private Identifier mapTextureId = null;
    ...
    private static int[] scanMap(Level level, BlockPos center, int mapSize) {
        ...
        for (int dz = -half; dz < mapSize - half; dz++) {
            for (int dx = -half; dx < mapSize - half; dx++) {
                ...
                // 使用 MOTION_BLOCKING_NO_LEAVES 更接近原版地图效果（包含水面，忽略树叶）
                ...
                // 高度明暗（立体感）：阈值降为 0 让高度差 1 格就有变化
                int diff = surfaceY - center.getY();
                if (diff > 0) argb = brighten(argb, Math.min(50, diff * 4));
                else if (diff < 0) argb = darken(argb, Math.min(50, -diff * 4));
    ...
    // ==================== 渲染 ====================
    public final ForgeLayer MINIMAP_LAYER = (guiGraphics, deltaTracker) -> {
        ...
        if (mapTextureId != null && textureSize == mapSize) {
            // 1.21.11 blit 签名：(Identifier, x, y, x2, y2, u0, u1, v0, v1)
            // UV 参数顺序：u0, u1, v0, v1（归一化 0~1），不是 u0, v0, u1, v1
            guiGraphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, mapTextureId, mapX, mapY, 0f, 0f, mapSize, mapSize, mapSize, mapSize);
        } else {
            // 还没有纹理
            guiGraphics.centeredText(mc.font, "§7加载中...",
    ...
    private void uploadTexture(Minecraft mc, int[] colors, int size) {
        ...
        if (mapTexture == null || textureSize != size) {
            ...
            mapTexture = new DynamicTexture("mchelper_minimap", size, size, false);
            mapTextureId = net.minecraft.resources.Identifier.fromNamespaceAndPath(
        ...
        for (int i = 0; i < size * size; i++) {
            int argb = colors[i];
            // setPixel 内部已调用 ARGB.toABGR() 转换，直接传 ARGB 值即可
            img.setPixel(i % size, i / size, argb);
        }
    ...
    // ==================== 玩家箭头 ====================
    private static void drawPlayerArrow(GuiGraphicsExtractor g, int cx, int cy, float yaw) {
