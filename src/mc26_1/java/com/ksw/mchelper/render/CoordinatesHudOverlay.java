...
public class CoordinatesHudOverlay {
    ...
    public static final ForgeLayer LAYER = (guiGraphics, deltaTracker) -> {
        ...
        if (MCHelperConfig.showTimeWeather) {
            long dayTime = mc.level.getLevelData().getGameTime() % 24000L;
        ...
        guiGraphics.text(mc.font, "§b\u25C6 MC Helper", textX, textY, TITLE_COLOR, true);
        textY += TITLE_HEIGHT;
        ...
        for (String[] line : lines) {
            ...
            guiGraphics.text(mc.font, label, textX, textY, LABEL_COLOR, true);

            // 值（白色）
            int labelWidth = mc.font.width(line[0] + ": ");
            guiGraphics.text(mc.font, "§f" + line[1], textX + labelWidth, textY, VALUE_COLOR, true);
     ...
     * 绘制渐变矩形（垂直方向）
     */
    private static void drawGradientRect(GuiGraphicsExtractor guiGraphics, int x1, int y1, int x2, int y2,
     ...
     * 绘制边框
     */
    private static void drawBorder(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height, int color) {
    ...
    private static String getBiomeDisplayName(Holder<Biome> biomeHolder) {
        ...
        if (key.isPresent()) {
            Identifier loc = key.get().identifier();
