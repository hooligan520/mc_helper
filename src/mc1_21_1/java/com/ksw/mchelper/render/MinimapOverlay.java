...
public class MinimapOverlay {
    ...
    public final LayeredDraw.Layer MINIMAP_LAYER = (guiGraphics, deltaTracker) -> {
        ...
        if (mapTextureId != null && textureSize == mapSize) {
            // 1.21.1: blit(ResourceLocation, x, y, destW, destH, textureW, textureH)
            guiGraphics.blit(mapTextureId, mapX, mapY, mapSize, mapSize, mapSize, mapSize);
        } else {
            // 还没有纹理
    ...
    private void uploadTexture(Minecraft mc, int[] colors, int size) {
        ...
        for (int i = 0; i < size * size; i++) {
            int argb = colors[i];
            // 1.21.1: setPixelRGBA 需要 ABGR 格式，手动转换
            int a = (argb >> 24) & 0xFF;
            int r = (argb >> 16) & 0xFF;
            int g = (argb >> 8)  & 0xFF;
            int b = argb & 0xFF;
            img.setPixelRGBA(i % size, i / size, (a << 24) | (b << 16) | (g << 8) | r);
        }
